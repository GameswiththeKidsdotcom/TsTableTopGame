...
|-----------|--------|-----------------|
| Plan location | Gap | Plan file must live at `.cursor/Plans/P004/P004-power-up.plan.md` (P003 already used for AI animated drop) |
| Sub-plan structure | Gap | Split into sub-plan files under `.cursor/Plans/P004/`; main plan stays short index |
...
|---------|-----------|---------------|-------------|---------------|--------------------------|----------------------------|
| P004 | Power-up System | 6 | Purchase power-ups with cash; hold 1; Clear Row, Send Garbage, Double Cash. AI same rules. Plan: [.cursor/Plans/P004/P004-power-up.plan.md](.cursor/Plans/P004/P004-power-up.plan.md). Sub-plans: [.cursor/Plans/P004/](.cursor/Plans/P004/) (P1–P5). | Pending analysis | N/A | N/A |

**Plan file paths (create):**
- Main: `.cursor/Plans/P004/P004-power-up.plan.md` (short index)
- Sub-plans: `.cursor/Plans/P004/P1-model.plan.md`, `P2-effects.plan.md`, `P3-ui.plan.md`, `P4-ai.plan.md`, `P5-tests.plan.md`
...
**P004 P1 — Model and stubs.** Plan: [.cursor/Plans/P004/P004-power-up.plan.md](.cursor/Plans/P004/P004-power-up.plan.md). Sub-plan: [.cursor/Plans/P004/P1-model.plan.md](.cursor/Plans/P004/P1-model.plan.md). Builder: Create `PowerUpType.swift`; add `powerUpHeld`, `doubleCashNextLockForPlayer` to GameState with `tryPurchasePowerUp`/`tryUsePowerUp` stubs; add `GridState.clearRow` and `lowestFullRow`. Unit test: purchase with sufficient cash succeeds and deducts. Investigator: validate P004 plan for 90%+ confidence before implementation. Expected: Model in place; purchase stub works; no UI yet.
...
1. **Move plan to workspace:** `.cursor/Plans/P004/P004-power-up.plan.md` (main index only)
2. **Create sub-plan files** under `.cursor/Plans/P004/` with full content, Confidence section, and matching cut-and-paste prompt
