...
**Next hand off (cut & paste)**: **P006 G1 — Level selection UI.** [P006-G1-level-selection.plan.md](P006-G1-level-selection.plan.md). Investigator: validate G1 for 90%+ confidence. Builder: add level picker to MenuView or Settings; store in SettingsManager; pass level from GameView/GameScene to `GameState(level:)`. Unit test virus count per level. Reference: [features-for-planner.md](../features-for-planner.md).
