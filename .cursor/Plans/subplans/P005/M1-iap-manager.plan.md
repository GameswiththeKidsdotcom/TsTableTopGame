...
**Next hand off (cut & paste)**: **P005 M1 — IAPManager.** [M1-iap-manager.plan.md](.cursor/Plans/subplans/P005/M1-iap-manager.plan.md). Investigator: validate M1 for 90%+ confidence. Builder: create `IAPManager` wrapping StoreKit 2; `IAPProductIDs`; entitlement check via `Transaction.currentEntitlements`. Unit test with StoreKit Configuration (M4 provides config file).
