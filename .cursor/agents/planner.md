...
- **Main plan (index)**: `.cursor/Plans/<plan-id>-<short-name>.plan.md` — short; lists phases/chunks and **links** to sub-plan files. No long step-by-step content here.
...
- **Each sub-plan file**: Full content for that slice only: steps, technical detail, validation, rollback, confidence. Self-contained so an agent can execute or validate that slice without loading the whole plan.
...
5. **Update or create plan docs in the proper location**: Create or update plan documents at **`.cursor/Plans/<plan-id>-<short-name>.plan.md`** (main plan) and **`.cursor/Plans/<plan-id>/<sub-id>-<short-name>.plan.md`** (sub-plans). For large plans, create sub-plan files so no single file exceeds a comfortable context size; keep the main plan as a short index that links to sub-plans. Each sub-plan document must contain full analysis, confidence, rollback, and validation for its slice. Link each plan from the Master-Plan; ensure every linked file exists and paths are correct. You own these documents and their accuracy.
